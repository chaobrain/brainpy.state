# examples/nest_like/iaf_tum_2000_short_term_depression.py
# Copyright 2026 BrainX Ecosystem Limited. Apache 2.0.
r"""Short-term depression in iaf_tum_2000 — NEST-style port.

Port of NEST's ``iaf_tum_2000_short_term_depression.py``, reproducing Figure 1A
of Tsodyks, Pawelzik & Markram (1998). The ``iaf_tum_2000`` neuron integrates
Tsodyks-Markram short-term plasticity *inside the presynaptic neuron*: each
presynaptic spike uses a fraction ``u`` of the available synaptic resources
``x``, and ``U`` sets how much ``u`` grows per spike. With a large ``U`` and no
facilitation (``tau_fac = 0``) the synaptic efficacy ``u * x`` saturates fast and
then *depresses* as the readily-releasable pool depletes.
...
potential is recorded. The successive EPSP amplitudes *decrease* — the signature
...
Run:  PYTHONPATH=. python examples/nest_like/iaf_tum_2000_short_term_depression.py
"""
import jax
import brainstate
...
# Membrane parameters (Tsodyks-Pawelzik-Markram 1998, Fig 1A; depression regime).
TAU_M = 40.0                       # membrane time constant [ms]
R_M = 0.1                          # membrane input resistance [GΩ]
C_M = TAU_M / R_M                  # membrane capacitance [pF] (= 400)
V_TH = 15.0                        # threshold potential [mV]
V_RESET = 0.0                      # reset / resting potential [mV]
T_REF = 2.0                        # refractory period [ms]

# Short-term plasticity parameters (depression: large U, facilitation off).
X0 = 1.0                           # initial readily-releasable fraction
U0 = 0.0                           # initial release probability
U = 0.5                            # increase of u per spike
TAU_PSC = 3.0                      # PSC decay constant [ms]
TAU_REC = 800.0                    # recovery from depression [ms]
TAU_FAC = 0.0                      # facilitation time constant (0 = off) [ms]
...
def build(simtime=T_SIM):
    """Build the two-neuron short-term-depression network.
...
def main():
    print('iaf_tum_2000 short-term depression (brainpy.state, Fig 1A)')
    print(f'  DC amplitude {DC_AMP:.2f} pA -> ~20 Hz presynaptic train')
    t, v = run_traces()
    # successive EPSP peaks decrease under depression
    ...
    try:
        ...
        plt.title('iaf_tum_2000 — short-term depression')
        plt.tight_layout()
        plt.savefig('examples/nest_like/iaf_tum_2000_short_term_depression.png', dpi=100)
        print('  wrote examples/nest_like/iaf_tum_2000_short_term_depression.png')
    except ImportError:
