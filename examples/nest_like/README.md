...
```bash
python examples/nest_like/brunel_alpha.py
```
...
any directly, e.g. `python examples/nest_like/pulsepacket.py`.
...
`python examples/nest_like/spatial_gaussex.py`):
...
`python examples/nest_like/two_comps.py`.
...
- **`pong_run.py`** — the `AIPong` host loop (a faithful port of NEST's `run_simulations.py`)
  ...
  `python examples/nest_like/pong_run.py --quick`.
...
Run `PYTHONPATH=. python examples/nest_like/sudoku.py --quick` for a fast smoke solve, or
