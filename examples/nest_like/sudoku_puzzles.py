...
brainpy spiking solver in :mod:`examples.nest_like.sudoku_net` can be measured against the
same configurations and the same notion of "valid".

See Also
--------
examples.nest_like.sudoku_net : the spiking WTA constraint network that relaxes a puzzle.
...
def get_puzzle(puzzle_index):
    ...
    .. code-block:: python
        ...
        >>> from examples.nest_like.sudoku_puzzles import get_puzzle
        >>> puzzle = get_puzzle(4)
        >>> puzzle.shape
        (9, 9)
        >>> puzzle[0].tolist()
...
def make_easy_puzzle(n_blank=12, seed=0):
    ...
    .. code-block:: python
        ...
        >>> from examples.nest_like.sudoku_puzzles import make_easy_puzzle, validate_solution
