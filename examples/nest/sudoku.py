...
:class:`~examples.nest.sudoku_net.SudokuNet` (3645 ``iaf_psc_exp`` neurons whose
...
Run::
    ...
    PYTHONPATH=. python examples/nest/sudoku.py --quick
    PYTHONPATH=. python examples/nest/sudoku.py --puzzle easy --seeds 5
    PYTHONPATH=. python examples/nest/sudoku.py --puzzle 4 --max-iterations 100
...
from examples.nest.sudoku_net import SudokuNet, DEFAULT_MAX_ITERATIONS, DEFAULT_SIM_TIME_MS
...
def solve_puzzle(puzzle, *, net=None, seed=0, pop_size=5,
    ...
    The instrumented twin of :meth:`~examples.nest.sudoku_net.SudokuSolver.solve`: it
...
def solve_seeds(puzzle, *, label='puzzle', seeds=1, pop_size=5,
    ...
    One :class:`~examples.nest.sudoku_net.SudokuNet` is built and reused across every
...
def main():
    ...
    parser.add_argument('--out', type=str, default='examples/nest/sudoku.png',
