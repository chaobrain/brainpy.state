...
"""A Brunel network with astrocytes, fixed-indegree wiring (NEST port) -- BLOCKED.

This is a documented placeholder, *not* a runnable port. NEST's
``astrocyte_brunel_fixed_indegree`` builds a balanced random (Brunel) network whose
...
``fixed_indegree`` rule (each neuron receives a fixed number ``CE``/``CI`` of
inputs), and the astrocyte third-factor edges use the
...
The sibling ``astrocyte_brunel_bernoulli`` is identical except its *primary* rule is
``pairwise_bernoulli``; both share the same astrocyte-pool blocker. What *is*
...
BLOCKED_REASON = (
    "astrocyte_brunel_fixed_indegree is blocked on nest.TripartiteConnect with the "
    "third_factor_bernoulli_with_pool astrocyte-pool connectivity rule "
    "(pool_size=10, pool_type='random'; primary rule fixed_indegree): the "
