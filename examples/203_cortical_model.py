...
class ExpSyn(bp.synapses.TwoEndConn):
  # Synapses parameters
  ...
  def __init__(self, pre, post, prob, syn_type='e', conn_type=0):
    ...
    # variables
    self.pre_sps = bp.VarDelay(pre.num, self.delay, bool)
...
class ThalamusInput(bp.synapses.TwoEndConn):
...
runner = bp.DSRunner(net, monitors=sps_monitors)
runner.run(1000.)
