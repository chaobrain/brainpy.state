...
def _facilitate_ref(*, w, kplus, lambda_, mu):
    power_term = float(np.power(np.float64(w), np.float64(mu)))
    return w + (lambda_ * power_term * kplus)


def _depress_ref(*, w, kminus, alpha, lambda_):
    new_w = w - (alpha * lambda_ * w * kminus)
    return new_w if new_w > 0.0 else 0.0


def _stdp_pl_hom_send_ref(
    *,
    weight,
    Kplus,
    t_lastspike,
    ...
    for t_post in _history_window_ref(post_hist_t, t_lastspike - delay, t_spike - delay):
        ...
        weight = _facilitate_ref(w=weight, kplus=kplus_term, lambda_=lambda_, mu=mu)
...
def _run_reference_weight_trace(
    *,
    ...
    for step in range(sim_steps):
        ...
        pre_count = pre_counts.get(step, 0)
        if pre_count > 0:
            current_weight, Kplus, t_lastspike, payload = _stdp_pl_hom_send_ref(
...
class TestSTDPPLSynapseHomParameters(unittest.TestCase):
    ...
    def test_nest_like_defaults(self):
        ...
        self.assertEqual(params['lambda'], 0.1)
        self.assertEqual(params['alpha'], 1.0)
        self.assertEqual(params['mu'], 0.4)
        self.assertEqual(params['Kplus'], 0.0)
        self.assertEqual(params['synapse_model'], 'stdp_pl_synapse_hom')
        self.assertAlmostEqual(syn.t_lastspike, 0.0, delta=1e-12)

    def test_common_properties_rejected_in_connect_syn_spec(self):
        syn = stdp_pl_synapse_hom()
        for key in ('tau_plus', 'lambda', 'alpha', 'mu'):
            with self.subTest(key=key):
                with self.assertRaisesRegex(ValueError, f'{key} cannot be specified'):
                    syn.check_synapse_params({'synapse_model': 'stdp_pl_synapse_hom', key: 1.0})

        syn.check_synapse_params({'synapse_model': 'stdp_pl_synapse_hom', 'weight': 3.0})
        syn.check_synapse_params({'synapse_model': 'stdp_pl_synapse_hom', 'delay': 2.0})
        syn.check_synapse_params({'synapse_model': 'stdp_pl_synapse_hom', 'Kplus': -2.0})

    def test_status_validation_matches_nest_hom_semantics(self):
        with brainstate.environ.context(dt=1.0 * u.ms, t=0.0 * u.ms):
            syn = stdp_pl_synapse_hom(weight=-1.0, Kplus=-0.5)
            syn.init_state()
            syn.update(pre_spike=0.0, post_spike=0.0)
            params = syn.get()
        self.assertEqual(params['weight'], -1.0)
        ...
        syn.set(weight=2.0, Kplus=-1.25)
        syn.set_weight(-7.5)
        params = syn.get()
        self.assertEqual(params['weight'], -7.5)
        self.assertEqual(params['Kplus'], -1.25)

        with self.assertRaisesRegex(ValueError, 'tau_plus must be > 0'):
            syn.set(tau_plus=0.0 * u.ms)
...
class TestSTDPPLSynapseHomVsNEST(unittest.TestCase):
    @staticmethod
    def _is_nest_available():
        try:
            import nest
            if hasattr(nest, 'synapse_models'):
                return 'stdp_pl_synapse_hom' in nest.synapse_models
            return 'stdp_pl_synapse_hom' in nest.Models()
        except Exception:
            return False
    ...
    def _run_nest_weight_trace(
        *,
        ...
        model_name = f'stdp_pl_synapse_hom_bpstate_{np.random.randint(1_000_000_000)}'
        nest.CopyModel(
            'stdp_pl_synapse_hom',
            model_name,
            {
