...
"""Spike-driven conductance parity for ``iaf_cond_exp_sfa_rr`` (goal 25, exp kinetics).

``iaf_cond_exp_sfa_rr`` (conductance LIF with spike-frequency adaptation + relative
refractory) was only ever validated with constant-current (``I_e``) drive -- never with
...
self-pull). Subthreshold drive keeps the SFA / relative-refractory conductances inactive so the
synaptic conductance traces match NEST cleanly.
"""
import gc
import unittest
...
VM_TOL = TraceTolerance(1e-3 * u.mV, 1e-3, align_steps=3, label='A',
                        note='iaf_cond_exp_sfa_rr V_m (spike-driven) vs live NEST')
COND_TOL = TraceTolerance(1e-3, 1e-3, align_steps=3, label='A',
                          note='iaf_cond_exp_sfa_rr g_ex/g_in vs live NEST')

#: Subthreshold weights (V_th=-57): V_m clearly departs E_L but never spikes, so the
#: SFA/relative-refractory conductances stay inert and the synaptic trace matches NEST.
...
INH_W = 25.0         # nS per spike (receptor 2 / NEST -weight); E_in=-75
SIM_T = 60.0
...
def _bp_drive(spike_times, weight, receptor_type, sim_time=SIM_T):
    """brainpy: spike_generator -> iaf_cond_exp_sfa_rr; return (times, V_m, g_ex, g_in)."""
    brainstate.environ.set(dt=DT * u.ms)
    sim = bp.Simulator(dt=DT * u.ms)
    n = sim.create(bp.iaf_cond_exp_sfa_rr, 1)
    if len(spike_times):
...
class TestIafCondExpSfaRrConductanceLaw(unittest.TestCase):
...
def _nest_drive(spike_times, weight_signed, sim_time=SIM_T):
    """NEST: spike_generator -> iaf_cond_exp_sfa_rr; return (V_m, g_ex, g_in)."""
    nest.set_verbosity('M_ERROR')
    nest.ResetKernel()
    nest.SetKernelStatus({'resolution': DT})
    n = nest.Create('iaf_cond_exp_sfa_rr', 1)
...
@requires_nest
class TestIafCondExpSfaRrConductanceParity(unittest.TestCase):
