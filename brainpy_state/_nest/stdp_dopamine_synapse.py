...
* a **broadcast dopamine concentration** ``n`` read every step from a bound
  :class:`~brainpy_state._nest.volume_transmitter` via the cluster-08
...
class stdp_dopamine_synapse:
    ...
    (broadcast from a :class:`~brainpy_state._nest.volume_transmitter`):
    ...
    *synapse* (integrated from the relayed dopa train). Here the spec is
    ...
    onto the broadcast :class:`~brainpy_state._nest.volume_transmitter`
