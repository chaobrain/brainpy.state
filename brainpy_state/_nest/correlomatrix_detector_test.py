...
def _run_bp_correlomatrix(
    ...
    if channel_weights is None:
        channel_weights = np.ones((n_channels,), dtype=np.float64)
    else:
            ...
            with brainstate.environ.context(t=step * dt):
                if stamp_step in schedule:
                    ...
                    cmd.update(
                        ...
                        weights=np.asarray([channel_weights[e[0]] for e in events], dtype=np.float64),
    ...
    return {
        'covariance': np.asarray(cmd.get('covariance'), dtype=np.float64),
        'count_covariance': np.asarray(cmd.get('count_covariance'), dtype=np.int64),
        'n_events': np.asarray(cmd.get('n_events'), dtype=np.int64),
    }
...
def _run_nest_correlomatrix(
    ...
    if channel_delays is None:
        channel_delays = np.ones((n_channels,), dtype=np.float64)
    else:
        channel_delays = np.asarray(channel_delays, dtype=np.float64)
...
class TestCorrelomatrixDetector(unittest.TestCase):
    ...
    def test_default_parameters_and_matrix_shape(self):
        ...
        npt.assert_array_equal(cmd.get('n_events'), np.array([0], dtype=np.int64))
        npt.assert_allclose(cmd.get('covariance'), np.zeros((1, 1, 11), dtype=np.float64), atol=1e-12)
        npt.assert_array_equal(cmd.get('count_covariance'), np.zeros((1, 1, 11), dtype=np.int64))
    ...
    def test_known_covariance_from_nest_testsuite(self):
        ...
        npt.assert_array_equal(out['n_events'], np.array([3, 2], dtype=np.int64))

        npt.assert_array_equal(out['count_covariance'][0, 1], np.array([1, 0, 1, 0, 2], dtype=np.int64))
        npt.assert_array_equal(out['count_covariance'][1, 0], np.array([1, 0, 1, 0, 0], dtype=np.int64))
        npt.assert_array_equal(out['count_covariance'][0, 0], np.array([3, 0, 1, 0, 1], dtype=np.int64))
        npt.assert_array_equal(out['count_covariance'][1, 1], np.array([2, 0, 0, 0, 1], dtype=np.int64))

        npt.assert_allclose(out['covariance'][0, 1], np.array([1, 0, 1, 0, 2], dtype=np.float64), atol=1e-12)
        npt.assert_allclose(out['covariance'][1, 0], np.array([1, 0, 1, 0, 0], dtype=np.float64), atol=1e-12)
    ...
    def test_reset_on_n_channels_change(self):
        with brainstate.environ.context(dt=self.dt):
            ...
            npt.assert_array_equal(cmd.get('n_events'), np.zeros((3,), dtype=np.int64))
            npt.assert_allclose(cmd.get('covariance'), np.zeros((3, 3, 5), dtype=np.float64), atol=1e-12)
            npt.assert_array_equal(cmd.get('count_covariance'), np.zeros((3, 3, 5), dtype=np.int64))
    ...
    def test_matches_nest_weighted_covariance_and_windows(self):
        ...
        channel_weights = np.array([1.7, -0.8, 0.5], dtype=np.float64)
        channel_delays = np.array([1.0, 1.0, 1.0], dtype=np.float64)
