...
def _run_nest_driven_trace(mode, dt_ms, simtime_ms, drive, weight, delay_ms):
    import nest
...
class TestRateNeuronIPN(unittest.TestCase):
    ...
    def test_step_equations_match_template_update_order_linear_summation_true(self):
        ...
        with brainstate.environ.context(dt=self.dt):
            ...
            for k in range(noise_seq.size):
                ...
                H_ex = float(mult_ex(rate_ref))
                H_in = float(mult_in(rate_ref))
                rate_new += P2 * H_ex * float(input_nl(delayed_ex + instant_ex))
                rate_new += P2 * H_in * float(input_nl(delayed_in + instant_in))
    ...
    def test_default_linear_template_instantaneous_and_delayed_driven_trace_matches_nest(self):
        ...
        nest_instant = _run_nest_driven_trace(
            mode='instantaneous',
            dt_ms=self.dt_ms,
            simtime_ms=simtime_ms,
            drive=drive,
        ...
        nest_delayed = _run_nest_driven_trace(
            mode='delayed',
            dt_ms=self.dt_ms,
            simtime_ms=simtime_ms,
            drive=drive,
        ...
        with brainstate.environ.context(dt=self.dt):
            bp_instant = rate_neuron_ipn(1, tau=5.0 * u.ms, lambda_=1.0, mu=0.0, sigma=0.0)
            bp_delayed = rate_neuron_ipn(1, tau=5.0 * u.ms, lambda_=1.0, mu=0.0, sigma=0.0)
