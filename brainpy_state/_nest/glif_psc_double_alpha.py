...
class glif_psc_double_alpha(Neuron):
    ...
    by the sum of two alpha functions (fast and slow components) for each
    receptor [2]_. This function is normalized such that an event of weight
    1.0 results in a peak current of the fast component of the alpha function
    to be 1 pA at :math:`t = \tau_\mathrm{syn,fast}`. The relative peak current
    of the slow component is given as ``amp_slow``, at
    :math:`t = \tau_\mathrm{syn,slow}`. Namely,

    .. math::

        I_\mathrm{syn} = \alpha(\tau_\mathrm{syn,fast})
                       + \mathrm{amp\_slow} \cdot \alpha(\tau_\mathrm{syn,slow})

    Therefore if ``amp_slow`` is not 0, the peak current of the total synaptic
    current is larger than the specified weight. By default,
    ``glif_psc_double_alpha`` has a single synapse that is accessible through
    ``receptor_port`` 1. An arbitrary number of synapses with different time
    constants and ``amp_slow`` can be configured by setting the desired
    parameters of ``tau_syn_fast``, ``tau_syn_slow``, and ``amp_slow`` arrays.
    ...
    * **GLIF Model 5** (LIF_R_ASC_A) — LIF with reset rules, after-spike
    ...
    Synaptic currents (double alpha function)
    .........................................

    Each receptor port has two current components (fast and slow), each modeled
    by an alpha function with two state variables :math:`y_1` and :math:`y_2`:
    ...
    .. math::
       ...
       P_{11,k}^\mathrm{fast} = P_{22,k}^\mathrm{fast} = \exp(-dt / \tau_{\mathrm{syn,fast},k})

       P_{21,k}^\mathrm{fast} = dt \cdot P_{11,k}^\mathrm{fast}

       P_{11,k}^\mathrm{slow} = P_{22,k}^\mathrm{slow} = \exp(-dt / \tau_{\mathrm{syn,slow},k})
    ...
    .. math::
       ...
       I_\mathrm{syn} = \sum_k \left( y_{2,k}^\mathrm{fast} + y_{2,k}^\mathrm{slow} \right)
    ...
    2. If not refractory:
       ...
       c. Update membrane potential (include both fast and slow synaptic
          contributions).
    ...
    4. Update synaptic current state variables for both fast and slow
       components.
    5. Add incoming spike current jumps (scaled appropriately for fast/slow).
    ...
    **Parameter**                   **Default**                **Math equivalent**                        **Description**
    ...
    ``amp_slow``                    (0.3,)                                                                Relative amplitude of slow component
    ...
    **State variable**           **Description**
    ...
    ``y1_fast``                  Fast synaptic current derivative state (list per receptor)
    ``y2_fast``                  Fast synaptic current state (list per receptor)
    ``y1_slow``                  Slow synaptic current derivative state (list per receptor)
    ``y2_slow``                  Slow synaptic current state (list per receptor)
    ...
    - Unlike ``glif_cond`` which uses an RKF45 ODE integrator, ``glif_psc_double_alpha``
      uses exact integration via propagator matrices for the linear subthreshold
      dynamics, matching NEST's implementation.
    - If ``tau_m`` is very close to ``tau_syn_fast`` or ``tau_syn_slow``, the model
      numerically behaves as if they are equal, to avoid numerical instabilities
      (see NEST IAF_Integration_Singularity notebook).
    - The double alpha function allows more flexible control over synaptic current
      shape compared to single alpha function in ``glif_psc``.
    ...
    def get_I_syn(self):
        """Get the total synaptic current (sum of fast and slow components).

        Returns
        -------
        I_syn : array
            Total synaptic current in pA.
        """
        I_syn = 0.0 * u.pA
        for k in range(self._n_receptors):
    ...
    def get_I_syn_fast(self):
        """Get the fast component of synaptic current.

        Returns
        -------
        I_syn_fast : array
            Fast synaptic current in pA.
        """
        I_syn = 0.0 * u.pA
        for k in range(self._n_receptors):
    ...
    def get_I_syn_slow(self):
        """Get the slow component of synaptic current.

        Returns
        -------
        I_syn_slow : array
            Slow synaptic current in pA.
        """
        I_syn = 0.0 * u.pA
        for k in range(self._n_receptors):
