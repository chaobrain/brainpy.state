...
class aeif_cond_alpha_astro(NESTNeuron):
    ...
    **4. SIC delivery semantics**
    ...
    def __init__(
        ...
        # inhibition by weight *sign* (aeif_cond_alpha_astro.cpp handle(): weight>0 ->
        # spike_exc_, else spike_inh_); the Simulator expresses that as
        # ``receptor_type=1``/``2`` with positive nS weights.
        self.n_receptors = 2
    ...
    def update(self, x=0.0 * u.pA, w_by_rec=None):
        ...
        w_by_rec : ArrayLike or None, optional
            ...
            the excitatory conductance and column 1 (``receptor_type=2``) to the
            inhibitory conductance. When ``None`` (the legacy path), the jumps are
