...
def _run_nest_sigmoid_driven_trace(linear_summation, dt_ms, simtime_ms, drive, weight, g, beta, theta):
    import nest
    ...
    target = nest.Create('sigmoid_rate_ipn')
    target.set({
        'tau': 5.0,
        'lambda': 1.0,
        'mu': 0.0,
...
class TestSigmoidRate(unittest.TestCase):
    ...
    def test_parameter_validation(self):
        with self.assertRaises(ValueError):
            sigmoid_rate_ipn(1, tau=0.0 * u.ms)
        with self.assertRaises(ValueError):
            sigmoid_rate_ipn(1, lambda_=-1e-3)
        with self.assertRaises(ValueError):
            sigmoid_rate_ipn(1, sigma=-1e-3)
        with self.assertRaises(ValueError):
            sigmoid_rate_ipn(1, rectify_rate=-1e-3)
    ...
    def test_linear_summation_modes_match_nest(self):
        ...
        nest_linear_sum = _run_nest_sigmoid_driven_trace(
        ...
        nest_event_sum = _run_nest_sigmoid_driven_trace(
            linear_summation=False,
            dt_ms=self.dt_ms,
            simtime_ms=simtime_ms,
            drive=drive,
        ...
        with brainstate.environ.context(dt=self.dt):
            bp_linear_sum = sigmoid_rate_ipn(
                1,
                tau=5.0 * u.ms,
                lambda_=1.0,
                mu=0.0,
