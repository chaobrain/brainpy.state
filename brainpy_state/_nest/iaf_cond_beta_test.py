...
def _reference_step(state, params, x_next, w_step, dt):
    ...
    if w_step >= 0.0:
        dg_ex += _beta_norm_factor(params['tau_rise_ex'], params['tau_decay_ex']) * w_step
    else:
        dg_in += _beta_norm_factor(params['tau_rise_in'], params['tau_decay_in']) * (-w_step)
...
class TestIAFCondBeta(unittest.TestCase):
    def setUp(self):
    ...
    def test_nest_cpp_default_parameters(self):
        ...
        self.assertEqual(neuron.tau_rise_ex, 0.2 * u.ms)
        self.assertEqual(neuron.tau_decay_ex, 0.2 * u.ms)
        self.assertEqual(neuron.tau_rise_in, 2.0 * u.ms)
        self.assertEqual(neuron.tau_decay_in, 2.0 * u.ms)
        self.assertEqual(neuron.I_e, 0. * u.pA)

    def test_parameter_validation(self):
        with self.assertRaises(ValueError):
            iaf_cond_beta(1, C_m=0.0 * u.pF)
        with self.assertRaises(ValueError):
            iaf_cond_beta(1, tau_rise_ex=0.0 * u.ms)
        with self.assertRaises(ValueError):
            iaf_cond_beta(1, tau_decay_ex=0.0 * u.ms)
        with self.assertRaises(ValueError):
            iaf_cond_beta(1, tau_rise_in=0.0 * u.ms)
        with self.assertRaises(ValueError):
            iaf_cond_beta(1, tau_decay_in=0.0 * u.ms)
        with self.assertRaises(ValueError):
            iaf_cond_beta(1, t_ref=-0.1 * u.ms)
        with self.assertRaises(ValueError):
            iaf_cond_beta(1, V_reset=-55. * u.mV, V_th=-55. * u.mV)
    ...
    def test_spike_weights_split_and_use_beta_normalization(self):
        with brainstate.environ.context(dt=self.dt):
            neuron = iaf_cond_beta(
                1,
                V_th=1000. * u.mV,
                tau_rise_ex=0.5 * u.ms,
                tau_decay_ex=2.0 * u.ms,
    ...
    def test_equal_rise_decay_uses_alpha_limit_normalization(self):
        with brainstate.environ.context(dt=self.dt):
            neuron = iaf_cond_beta(1, V_th=1000. * u.mV)
            neuron.init_state()
            self._step(neuron, 0, dg_values=[2.0, -3.0])
