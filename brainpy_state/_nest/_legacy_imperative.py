...
# ---------------------------------------------------------------------------
# RELOCATED imperative synapse base (cluster 01).
#
# This module holds the original imperative ``static_synapse`` (a Python event
# queue + ``send()``/``update()`` host-scalar machinery), renamed to
# ``ImperativeSynapseBase``. It is the base class that the not-yet-rebuilt
# synapse clusters (STDP / bernoulli / cont_delay) still subclass. The public
# ``static_synapse`` (in ``static_synapse.py``) is now a pure spec+rule consumed
# by ``EventPlasticProj``.
#
# TODO(04/05/06): delete this module once the STDP/bernoulli/cont_delay clusters
# are rebuilt onto ``EventPlasticProj``.
# ---------------------------------------------------------------------------

__all__ = [
    'ImperativeSynapseBase',
    '_UNSET',
]
...
class ImperativeSynapseBase(NESTSynapse):
    ...
    **Non-spike event types:**
    ...
    def _delay_ms_to_steps(delay_ms: float, dt_ms: float) -> int:
        return ImperativeSynapseBase._ld_round(delay_ms / dt_ms)
