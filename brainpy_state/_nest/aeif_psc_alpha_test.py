...
def _rhs(y, is_refractory, i_stim, p):
    ...
    return np.asarray([dv, ddI_ex, dI_ex_dt, ddI_in, dI_in_dt, dw], dtype=np.float64)
...
class TestAEIFPscAlpha(unittest.TestCase):
    ...
    def test_direct_trace_matches_nest_if_available(self):
        ...
        with brainstate.environ.context(dt=dt_ms * u.ms):
            ...
            neuron.init_state()
            neuron.dI_ex.value = np.asarray([params['dI_syn_ex']], dtype=np.float64)
            neuron.dI_in.value = np.asarray([params['dI_syn_in']], dtype=np.float64)
