...
def _threshold(h, g, theta, alpha):
...
def _run_nest_threshold_driven_trace(linear_summation, dt_ms, simtime_ms, drive, weight, g, theta, alpha):
    import nest
    ...
    target = nest.Create('threshold_lin_rate_ipn')
    target.set({
        'tau': 5.0,
        'lambda': 1.0,
        'mu': 0.0,
...
class TestThresholdLinRate(unittest.TestCase):
    def setUp(self):
    ...
    def test_nest_default_parameters(self):
        ...
        self.assertTrue(np.isinf(np.asarray(u.math.asarray(ipn.alpha), dtype=np.float64)).all())
        ...
        self.assertTrue(np.isinf(np.asarray(u.math.asarray(opn.alpha), dtype=np.float64)).all())
    ...
    def test_parameter_validation(self):
        with self.assertRaises(ValueError):
            threshold_lin_rate_ipn(1, tau=0.0 * u.ms)
        with self.assertRaises(ValueError):
            threshold_lin_rate_ipn(1, lambda_=-1e-3)
        with self.assertRaises(ValueError):
            threshold_lin_rate_ipn(1, sigma=-1e-3)
        with self.assertRaises(ValueError):
            threshold_lin_rate_ipn(1, rectify_rate=-1e-3)

        with self.assertRaises(ValueError):
            threshold_lin_rate_opn(1, tau=0.0 * u.ms)
        with self.assertRaises(ValueError):
            threshold_lin_rate_opn(1, sigma=-1e-3)
        ...
        with brainstate.environ.context(dt=self.dt):
            ...
            for k in range(noise_seq.size):
                ...
                for ev in delayed_events_seq[k]:
                    val = float(ev['weight']) * float(_threshold(
                        float(ev['rate']),
                        params['g'],
                        params['theta'],
                        params['alpha'],
    ...
    def test_matches_nest_deterministic_trace_ipn_and_opn(self):
        ...
        ipn_nest = _run_nest_trace(
            model_name='threshold_lin_rate_ipn',
            params={
                'tau': 5.0,
                'lambda': 1.2,
                'sigma': 0.0,
        ...
        opn_nest = _run_nest_trace(
            model_name='threshold_lin_rate_opn',
            params={
                'tau': 7.0,
                'sigma': 0.0,
                'mu': -0.8,
    ...
    def test_linear_summation_modes_match_nest(self):
        ...
        nest_linear_sum = _run_nest_threshold_driven_trace(
        ...
        nest_event_sum = _run_nest_threshold_driven_trace(
            linear_summation=False,
            dt_ms=self.dt_ms,
            simtime_ms=simtime_ms,
            drive=drive,
        ...
        with brainstate.environ.context(dt=self.dt):
            bp_linear_sum = threshold_lin_rate_ipn(
                1,
                tau=5.0 * u.ms,
                lambda_=1.0,
                mu=0.0,
            ...
            bp_event_sum = threshold_lin_rate_ipn(
                1,
                tau=5.0 * u.ms,
                lambda_=1.0,
                mu=0.0,
