...
def _sigmoid_gg_1998(h, g):
    gh4 = (g * h) ** 4.0
    return gh4 / ((0.1 ** 4.0) + gh4)
...
def _run_nest_gg_1998_driven_trace(linear_summation, dt_ms, simtime_ms, drive, weight, g):
    import nest
    ...
    target = nest.Create('sigmoid_rate_gg_1998_ipn')
    target.set({
        'tau': 5.0,
        'lambda': 1.0,
        'mu': 0.0,
...
class TestSigmoidRateGG1998(unittest.TestCase):
    ...
    def test_nest_default_parameters(self):
        ipn = sigmoid_rate_gg_1998_ipn(1)
    ...
    def test_parameter_validation(self):
        with self.assertRaises(ValueError):
            sigmoid_rate_gg_1998_ipn(1, tau=0.0 * u.ms)
        with self.assertRaises(ValueError):
            sigmoid_rate_gg_1998_ipn(1, lambda_=-1e-3)
        with self.assertRaises(ValueError):
            sigmoid_rate_gg_1998_ipn(1, sigma=-1e-3)
        with self.assertRaises(ValueError):
            sigmoid_rate_gg_1998_ipn(1, rectify_rate=-1e-3)
    ...
    def test_ipn_step_equations_match_nest_update_ordering(self):
        ...
        with brainstate.environ.context(dt=self.dt):
            neuron = sigmoid_rate_gg_1998_ipn(
                1,
                tau=params['tau'] * u.ms,
            ...
            for k in range(noise_seq.size):
                ...
                for ev in delayed_events_seq[k]:
                    val = float(ev['weight']) * _sigmoid_gg_1998(float(ev['rate']), params['g'])
    ...
    def test_mult_coupling_flag_is_noop(self):
        ...
        with brainstate.environ.context(dt=self.dt):
            ...
            def run_n0(_):
                ...
                return n0.rate.value.reshape(-1)[0]

            def run_n1(_):
                n1.update(instant_rate_events=event, noise=0.0)
                return n1.rate.value.reshape(-1)[0]

            y0 = np.asarray(brainstate.transform.for_loop(run_n0, np.zeros(steps)))
            y1 = np.asarray(brainstate.transform.for_loop(run_n1, np.zeros(steps)))

        npt.assert_allclose(y0, y1, atol=1e-12)

    def test_matches_nest_deterministic_trace_ipn(self):
        ...
        ipn_nest = _run_nest_trace(
            model_name='sigmoid_rate_gg_1998_ipn',
            params={
                'tau': 5.0,
                'lambda': 1.2,
                'sigma': 0.0,
        ...
        with brainstate.environ.context(dt=self.dt):
            ipn_bp = sigmoid_rate_gg_1998_ipn(
                1,
                tau=5.0 * u.ms,
                lambda_=1.2,
                sigma=0.0,
    ...
    def test_linear_summation_modes_match_nest(self):
        ...
        nest_linear_sum = _run_nest_gg_1998_driven_trace(
        ...
        nest_event_sum = _run_nest_gg_1998_driven_trace(
            linear_summation=False,
            dt_ms=self.dt_ms,
            simtime_ms=simtime_ms,
            drive=drive,
        ...
        with brainstate.environ.context(dt=self.dt):
            bp_linear_sum = sigmoid_rate_gg_1998_ipn(
                1,
                tau=5.0 * u.ms,
                lambda_=1.0,
                mu=0.0,
            ...
            bp_event_sum = sigmoid_rate_gg_1998_ipn(
                1,
                tau=5.0 * u.ms,
                lambda_=1.0,
                mu=0.0,
            ...
            event = [{'rate': drive, 'weight': weight}]

            def run_linear(_):
                bp_linear_sum.update(instant_rate_events=event, noise=0.0)
                return bp_linear_sum.rate.value.reshape(-1)[0]

            def run_event(_):
                bp_event_sum.update(instant_rate_events=event, noise=0.0)
                return bp_event_sum.rate.value.reshape(-1)[0]

            y_linear_sum = np.asarray(brainstate.transform.for_loop(run_linear, np.zeros(steps)))
            y_event_sum = np.asarray(brainstate.transform.for_loop(run_event, np.zeros(steps)))
