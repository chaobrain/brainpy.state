    ...
    while t < dt_ms and iters < 100000:
        ...
        if err <= atol or h <= min_h:
            ...
            if y[0] < -1e3 or y[3] < -1e6 or y[3] > 1e6:
                raise ValueError('Numerical instability in reference aeif_psc_exp.')
...
class TestAEIFPscExp(unittest.TestCase):
    def setUp(self):
    ...
    def test_direct_trace_matches_nest_if_available(self):
        ...
        if 'aeif_psc_exp' not in nest.Models():
            self.skipTest('NEST model aeif_psc_exp not available')
        ...
        nrn = nest.Create('aeif_psc_exp', params=params)
