...
def _stdp_nn_restr_send_ref(
    *,
    ...
    if history:
        minus_dt = t_lastspike - (history[0] + delay)
...
def _run_reference_weight_trace(
    *,
    ...
    for step in range(sim_steps):
        ...
        pre_count = pre_counts.get(step, 0)
        if pre_count > 0:
            current_weight, t_lastspike, payload = _stdp_nn_restr_send_ref(
...
class TestSTDPNNRestrSynapseParameters(unittest.TestCase):
    def test_nest_like_defaults(self):
        ...
        self.assertEqual(params['synapse_model'], 'stdp_nn_restr_synapse')
    ...
    def test_parameter_validation_matches_nest_semantics(self):
        ...
        syn = stdp_nn_restr_synapse(weight=2.0, Wmax=5.0)
        with self.assertRaisesRegex(ValueError, 'Weight and Wmax must have same sign'):
            syn.set(weight=-1.0)
...
class TestSTDPNNRestrSynapseOrdering(unittest.TestCase):
...
class TestSTDPNNRestrSynapseDynamics(unittest.TestCase):
...
class TestSTDPNNRestrSynapseVsNEST(unittest.TestCase):
    @staticmethod
    def _is_nest_available():
        try:
            ...
            if hasattr(nest, 'synapse_models'):
                return 'stdp_nn_restr_synapse' in nest.synapse_models
            return 'stdp_nn_restr_synapse' in nest.Models()
        except Exception:
            return False
    ...
    def _run_nest_weight_trace(
        *,
        ...
        model_name = f'stdp_nn_restr_synapse_bpstate_{np.random.randint(1_000_000_000)}'
        nest.CopyModel(
