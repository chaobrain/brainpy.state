...
class _threshold_lin_rate_base(_lin_rate_base):
    ...
    #: φ(h) = min(max(g·(h − θ), 0), α) is fixed by the gain, threshold and ceiling;
    #: these identify it for the ``linear_summation=False`` homogeneity guard.
    _phi_param_names = ('g', 'theta', 'alpha')

    def _activation(self, h):
        """Threshold-linear gain ``φ(h) = min(max(g·(h − θ), 0), α)`` (JAX; reads ``self``)."""
        g = u.get_mantissa(self.g)
        theta = u.get_mantissa(self.theta)
        alpha = u.get_mantissa(self.alpha)
        return jnp.minimum(jnp.maximum(g * (h - theta), 0.0), alpha)
