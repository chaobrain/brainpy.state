...
"""Spike-driven conductance parity for ``iaf_cond_alpha`` (goal 25, alpha kinetics).

``iaf_cond_alpha`` was only ever validated with constant-current (``I_e``) drive -- never with
...
positive ``nS`` = NEST's weight-**sign** routing (``iaf_cond_alpha.cpp`` ``handle()``). The
*alpha* synapse applies ``dg_ex += (e/tau_syn_ex)*w_ex``; only the *source* of ``w_ex``/``w_in``
changed (bridge vs self-pull).
"""
import gc
import unittest
...
VM_TOL = TraceTolerance(1e-3 * u.mV, 1e-3, align_steps=3, label='A',
                        note='iaf_cond_alpha V_m (spike-driven) vs live NEST')
COND_TOL = TraceTolerance(1e-3, 1e-3, align_steps=3, label='A',
                          note='iaf_cond_alpha g_ex/g_in vs live NEST')

#: Moderate weights keep V_m clearly above E_L but subthreshold (V_th=-55) so the
#: alpha-shaped conductance trace matches NEST cleanly.
...
def _bp_drive(spike_times, weight, receptor_type, sim_time=SIM_T):
    """brainpy: spike_generator -> iaf_cond_alpha; return (times, V_m, g_ex, g_in)."""
    brainstate.environ.set(dt=DT * u.ms)
    sim = bp.Simulator(dt=DT * u.ms)
    n = sim.create(bp.iaf_cond_alpha, 1)
    if len(spike_times):
...
class TestIafCondAlphaConductanceLaw(unittest.TestCase):
...
def _nest_drive(spike_times, weight_signed, sim_time=SIM_T):
    """NEST: spike_generator -> iaf_cond_alpha; return (V_m, g_ex, g_in)."""
    nest.set_verbosity('M_ERROR')
    nest.ResetKernel()
    nest.SetKernelStatus({'resolution': DT})
    n = nest.Create('iaf_cond_alpha', 1)
...
@requires_nest
class TestIafCondAlphaConductanceParity(unittest.TestCase):
