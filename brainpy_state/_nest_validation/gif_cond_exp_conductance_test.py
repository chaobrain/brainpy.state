...
"""Spike-driven conductance parity for ``gif_cond_exp`` (goal 25, exp kinetics, stochastic).

``gif_cond_exp`` (conductance-based Generalised Integrate-and-Fire with *stochastic* spike
emission) was only ever validated with constant-current (``I_e``) drive -- never with
excitatory/inhibitory **spike** input into its synaptic conductance. That path was silently
dead: the Simulator never populated the ``label='w_ex'/'w_in'`` delta channels the model
...
GIF fires stochastically with hazard ``lambda_0 * exp((V_m - V_T_star) / Delta_V)``
(``V_T_star=-35 mV``, ``Delta_V=0.5 mV``). The weights here are deliberately small and
high-impedance ``gif_cond_exp`` (``g_L=4 nS``) is sensitive, so ``V_m`` peaks near ``-58 mV`` --
~23 mV below ``V_T_star``, where the hazard is ``< 1e-19`` per step. Both simulators therefore
stay silent (no RNG divergence) and the subthreshold conductance traces match deterministically.
"""
import gc
import unittest
...
VM_TOL = TraceTolerance(1e-3 * u.mV, 1e-3, align_steps=3, label='A',
                        note='gif_cond_exp V_m (spike-driven) vs live NEST')
COND_TOL = TraceTolerance(1e-3, 1e-3, align_steps=3, label='A',
                          note='gif_cond_exp g_ex/g_in vs live NEST')

#: Small weights into a high-impedance cell (g_L=4 nS): V_m peaks ~-58 mV, well below the
#: soft threshold V_T_star=-35 mV, so the stochastic hazard stays ~0 and neither simulator
#: spikes -- the subthreshold conductance traces stay deterministic and match NEST.
EXC_SPIKES = [10., 12., 14., 16., 18., 20., 22., 24.]
EXC_W = 0.5          # nS per spike (receptor 1 / NEST +weight)
INH_SPIKES = [10., 13., 16., 19., 22.]
INH_W = 5.0          # nS per spike (receptor 2 / NEST -weight); E_in=-85
SIM_T = 70.0
...
def _bp_drive(spike_times, weight, receptor_type, sim_time=SIM_T):
    """brainpy: spike_generator -> gif_cond_exp; return (times, V_m, g_ex, g_in)."""
    brainstate.environ.set(dt=DT * u.ms)
    sim = bp.Simulator(dt=DT * u.ms)
    n = sim.create(bp.gif_cond_exp, 1)
    if len(spike_times):
...
class TestGifCondExpConductanceLaw(unittest.TestCase):
    ...
    def test_excitatory_train_raises_g_ex_and_depolarises(self):
        ...
        self.assertLess(float(np.max(v)), -45.0, 'the drive stays subthreshold (no stochastic spike)')
...
def _nest_drive(spike_times, weight_signed, sim_time=SIM_T):
    """NEST: spike_generator -> gif_cond_exp; return (V_m, g_ex, g_in)."""
    nest.set_verbosity('M_ERROR')
    nest.ResetKernel()
    nest.SetKernelStatus({'resolution': DT})
    n = nest.Create('gif_cond_exp', 1)
...
@requires_nest
class TestGifCondExpConductanceParity(unittest.TestCase):
