...
"""Spike-driven conductance parity for ``aeif_cond_alpha`` (goal 25, alpha kinetics).

``aeif_cond_alpha`` was only ever validated with constant-current (``I_e``) drive -- never
...
This module validates the fix. ``aeif_cond_alpha`` now exposes the 17b multi-receptor bridge
...
-- the Simulator's expression of NEST's weight-**sign** routing (``aeif_cond_alpha.cpp``
``handle()``: ``weight>0 -> spike_exc_``, else ``spike_inh_``). The *alpha* synapse applies
the jump via ``dg_ex += (e/tau_syn_ex)*w_ex``; only the *source* of ``w_ex``/``w_in`` changed
(bridge vs self-pull), so this is the alpha-kinetics arbiter for design A (source-only suffices).
"""
import gc
import unittest
...
VM_TOL = TraceTolerance(1e-3 * u.mV, 1e-3, align_steps=3, label='A',
                        note='aeif_cond_alpha V_m (spike-driven) vs live NEST')
COND_TOL = TraceTolerance(1e-3, 1e-3, align_steps=3, label='A',
                          note='aeif_cond_alpha g_ex/g_in vs live NEST')

#: Closely-spaced excitatory train so the fast (0.2 ms) alpha excitatory conductance
#: accumulates and the membrane clearly departs from rest.
...
def _bp_drive(spike_times, weight, receptor_type, sim_time=SIM_T):
    """brainpy: spike_generator -> aeif_cond_alpha; return (times, V_m, g_ex, g_in)."""
    brainstate.environ.set(dt=DT * u.ms)
    sim = bp.Simulator(dt=DT * u.ms)
    n = sim.create(bp.aeif_cond_alpha, 1)
    if len(spike_times):
...
class TestAeifCondAlphaConductanceLaw(unittest.TestCase):
...
def _nest_drive(spike_times, weight_signed, sim_time=SIM_T):
    """NEST: spike_generator -> aeif_cond_alpha; return (V_m, g_ex, g_in)."""
    nest.set_verbosity('M_ERROR')
    nest.ResetKernel()
    nest.SetKernelStatus({'resolution': DT})
    n = nest.Create('aeif_cond_alpha', 1)
...
@requires_nest
class TestAeifCondAlphaConductanceParity(unittest.TestCase):
