...
"""Spike-driven conductance parity for ``iaf_cond_beta`` (goal 25, beta dual-exp kinetics).

``iaf_cond_beta`` was only ever validated with constant-current (``I_e``) drive -- never with
...
This module validates the fix. ``iaf_cond_beta`` now exposes the 17b multi-receptor bridge
...
-- the Simulator's expression of NEST's weight-**sign** routing (``iaf_cond_beta.cpp``
``handle()``: ``weight>0 -> spike_exc_``, else ``spike_inh_``). The *beta* synapse applies the
jump via ``dg_ex += pscon_beta*w_ex`` where ``pscon_beta`` is the dual-exp normalization factor;
only the *source* of ``w_ex``/``w_in`` changed (bridge vs self-pull), so this is the
beta-kinetics arbiter for design A (source-only suffices; the dual-exp normalization is untouched).
"""
import gc
import unittest
...
VM_TOL = TraceTolerance(1e-3 * u.mV, 1e-3, align_steps=3, label='A',
                        note='iaf_cond_beta V_m (spike-driven) vs live NEST')
COND_TOL = TraceTolerance(1e-3, 1e-3, align_steps=3, label='A',
                          note='iaf_cond_beta g_ex/g_in vs live NEST')

#: Excitatory train: moderate weight keeps V_m clearly above E_L but subthreshold
#: (V_th=-55, 15 mV headroom) so the beta-normalized conductance trace matches cleanly.
...
def _bp_drive(spike_times, weight, receptor_type, sim_time=SIM_T):
    """brainpy: spike_generator -> iaf_cond_beta; return (times, V_m, g_ex, g_in)."""
    brainstate.environ.set(dt=DT * u.ms)
    sim = bp.Simulator(dt=DT * u.ms)
    n = sim.create(bp.iaf_cond_beta, 1)
    if len(spike_times):
...
class TestIafCondBetaConductanceLaw(unittest.TestCase):
...
def _nest_drive(spike_times, weight_signed, sim_time=SIM_T):
    """NEST: spike_generator -> iaf_cond_beta; return (V_m, g_ex, g_in)."""
    nest.set_verbosity('M_ERROR')
    nest.ResetKernel()
    nest.SetKernelStatus({'resolution': DT})
    n = nest.Create('iaf_cond_beta', 1)
...
@requires_nest
class TestIafCondBetaConductanceParity(unittest.TestCase):
