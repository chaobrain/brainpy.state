...
4. `ModelRef.kind` is registered (§7).
5. `ConnRule.kind` is registered (§6.1).
...
Violations raise `SpecError` with a node id and a precise message (§8.2).
...
- **Seeding.** Rules expose a `seed` kwarg. If unset, the projection's
  `seed` (or, failing that, the build-time seed; §8.1) is folded in via
