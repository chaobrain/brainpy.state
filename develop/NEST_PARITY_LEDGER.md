# develop/NEST_PARITY_LEDGER.md — NEST-parity goal ledger
...
1. **Synapse-rebuild stream.** Take every synapse/plasticity model in
   `develop/nest-status/internal/synapses-plasticity-gap.md` from
...
2. **Example-port stream.** Port every non-`unsupported` example in
   `develop/nest-status/internal/examples-gap.md` §3.1–3.10 to `examples/nest_like/<name>.py`,
...
4. **NEST fidelity wins.** Where NEST semantics and a brainpy.state convention
   ...
   adapter overrides and the delta is documented (docstring + `develop/NEST_PARITY_LEDGER.md`).
...
- **Already done (not a cluster):** §3.1 flagship Brunel family —
  `examples/nest_like/brunel_{alpha,delta,exp_multisynapse,siegert,alpha_evolution_strategies}.py`
...
- **Session protocol** (canonical text in the master index; each prompt embeds a
  ...
  2. **Read `develop/NEST_PARITY_LEDGER.md` fully**, then the named gap-doc sections + relevant source.
  ...
  6. **Append a Lessons entry to `develop/NEST_PARITY_LEDGER.md`** (Part 5 template).
...
- **Shipped:** the **§3.10 `sudoku/` port** — the one item cluster 19 deferred as
  "intractable". `examples/nest_like/sudoku.py` (runnable harness: `solve_puzzle`/`solve_seeds`
...
- **Shipped:** the **full residual `nest.spatial.*` surface** (network-api-gap §3.10 rows 157–163,
  ...
  **`nearest_element`/`select_nodes_by_mask`** + exporters **`dump_layer_nodes`/`dump_layer_connections`**
  ...
  (new `_plot.py`, matplotlib lazily imported). Demo `examples/nest_like/spatial_gabor.py` (gabor in a
...
- **Shipped:** the **eight remaining conductance neurons** enrolled into the 17b
  ...
  follow-up **cleared** + §3 eight rows flipped; `examples/nest_like/README.md` `multimeter_file`
  noted now-portable.
...
- **Shipped:** the two substrate-ready **§3.8 astrocyte demos** on the `Simulator` API +
  ...
  `examples/nest_like/astrocyte_single.py` (one Poisson-driven `astrocyte_lr_1994` → IP3/Ca + a
...
- **Shipped:** the two remaining §3.6 **rate-network demos** on the `Simulator` API, each a
  ...
  `examples/nest_like/lin_rate_ipn_network.py` (E/I `lin_rate_ipn`, delayed-E + instantaneous-I)
  and `examples/nest_like/rate_neuron_dm.py` (two-unit rectified WTA decision). Tests:
  `_nest/_validation/lin_rate_ipn_network_test.py` (8), `rate_neuron_dm_test.py` (8). Docs:
  `examples/nest_like/README.md` §3.6 + `develop/nest-status/internal/examples-gap.md` rows flipped.
...
- **Shipped:** the **Siegert mean-field node + dual-channel `diffusion_connection`** on the
  ...
  `examples/nest_like/brunel_siegert.py` Simulator rewrite. Branch
...
- **Shipped:** **explicit-lag gap-junction coupling** for the two gap-capable HH neurons,
  ...
  gap_junction_no_nest}_test.py`, `examples/nest_like/gap_junctions_{two_neurons,
...
- **Shipped:** the **three §3.7 generator-pattern demos**, each a runnable
  `examples/nest_like/` script + a single test module carrying **both** a NEST-free
  ...
  Files: `examples/nest_like/{sinusoidal_poisson_generator,sinusoidal_gamma_generator,pulsepacket}.py`,
  `brainpy_state/_nest/_validation/{sinusoidal_poisson_generator,sinusoidal_gamma_generator,pulsepacket}_test.py`,
  docs (`examples/nest_like/README.md` §3.7, `develop/nest-status/internal/examples-gap.md`).
...
- **Shipped:** the **last plastic synapse on the legacy base** — `urbanczik_synapse`
  ...
  `examples/nest_like/urbanczik_synapse_example.py` (Urbanczik-Senn 2014 Fig 1B) and retired
  ...
  `numerical-validation-gap.md`, `index.rst`, `examples/nest_like/README.md`). Branch
...
- **Shipped:** **recurrent NMDA coupling through the `Simulator` API + the Wang (2002)
  ...
  `examples/nest_like/wang_decision_making.py` (the deferred placeholder retired);
...
- **Shipped:** **post-hoc connection enumeration + introspection** —
  ...
  `examples/nest_like/plot_weight_matrices.py` (four `E→E/E→I/I→E/I→I` weight matrices via
  `get_connections(src,tgt)` + `np.add.at`) and `examples/nest_like/synapsecollection.py`
...
- **Shipped:** the **§3.6 spiking network-demo cluster** — 5 ports on the `Simulator`
  ...
  `examples/nest_like/{artificial_synchrony,repeated_stimulation,sensitivity_to_perturbation,
  ...
  benchmark-3 sibling) to the new IF benchmarks 1&2. Docs: `examples/nest_like/README.md`
...
- **Shipped:** the **§3.5 single-neuron model-demo cluster, part 2** — 9 more
  `examples/nest_like/` ports completing NEST §3.5 (now **16** total), each with a
...
- **Shipped:** the **§3.5 single-neuron model-demo cluster** — 7 ports under
  `examples/nest_like/` each with a live-NEST trace-parity test in `_nest/_validation/`:
  ...
  `examples/nest_like/README.md` §3.5 section + parity table, `examples-gap.md` §3.5 marked
...
- **Shipped:** the **four implementable §3.3 plasticity demos** on the `Simulator` API,
  ...
  README §3.3 / examples-gap §3.3 docs. Files: `examples/nest_like/{clopath_synapse_spike_pairing,
...
- **Gotchas:**
  ...
  - **Standalone examples need `PYTHONPATH=. python examples/nest_like/<f>.py`.** Running a script
...
- **Shipped:** repaired `brunel_alpha_evolution_strategies_test.py` (the lone red on `main`
  ...
  `examples/nest_like/brunel_alpha_evolution_strategies.py`. Repointed both imports to
  `from examples.nest_like.brunel_alpha_evolution_strategies import ...` (the house pattern its
...
- **Gotchas:**
  ...
  - **`examples` is importable from tests via pytest prepend-mode.** Repo root has no
    ...
    `sys.path` → `import examples.nest_like.<m>` resolves (`examples` is a PEP-420 namespace pkg;
    `examples/nest_like/__init__.py` exists). Verified under both `python -m pytest` and the bare
    `pytest` entrypoint CI uses.
  - **Band-aid vs fix:** when a port's test fails on a bad import, repoint it to the real source —
    do not delete the test.
- **For next clusters:** NEST-example ports import helpers `from examples.nest_like.<module>` (the
...
- **Shipped:** the 5 NEST §3.4 recording/device demos on the `Simulator` API
  (`examples/nest_like/{multimeter_file,recording_demo,cross_check_mip_corrdet,
...
- **Shipped:** the 7 NEST §3.2 single-/few-neuron demos on the `Simulator` API
  (`examples/nest_like/{one_neuron,one_neuron_with_noise,twoneurons,testiaf,
...
- **Gotchas (NEST fidelity + JAX):**
  ...
  - **Worktree import shadowing:** bare `python examples/nest_like/foo.py` picks up the
